$(document)
		.ready(
				function() {
					...
					$(window)
							.bind(
									canDetect ? "orientationchange" : "resize",
									function(evt) {
										...
										orientationTimer = setTimeout(
												function() {
													...
													$("#window-height").html(
															window.innerHeight);
													// given we can only really
													// rely on width and height
													// at this stage,
													...
													// based on aspect ratio
													var orientation = aspectRatio <= 1 ? "portrait"
															: "landscape";
													// if the event type is an
													// orientation change event,
													// we can rely on
													// the orientation angle
													var rotationText = null;
													if (evt.type == "orientationchange") {
														rotationText = ROTATION_CLASSES[window.orientation
																.toString()];
													} // if
													// display the details we
													// have determined from the
													// display
													$("#orientation").html(
															orientation);
													$("#rotation-class").html(
